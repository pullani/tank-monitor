...
void setup(){
  ...
  if(WiFi.ready())
  {
      Particle.publish("Wi-Fi Status",( "Connected Successfully to \"" + (String) WiFi.SSID() + "\"") ,PUBLIC);
  }
...
bool procData()
{
    if(data.startsWith("##") && data.endsWith("##"))
    {
      ...
      Serial.print(atof(val)); //will avoid char parts, unwanted junk - returns value as float
      Serial.println(" cm");
      errFlag = false;
    }
    else
...
int handleAlerts() //level values global variable used to avoid optimisations
{
    ...
    char jsonMsg[1024]; char buff_lvl[10];
    StaticJsonBuffer<1024> jsonBuffer;
    JsonObject& root = jsonBuffer.createObject();
    sprintf(buff_lvl, "%.2f", levelPer);
    root["level"] = (String) buff_lvl + "%";
    root["time"] = Time.format(Time.now(), "%b %e %a %I:%M:%S %p");
    root.printTo(jsonMsg); //Can access level and time as {{level}} and {{time}} in webhook integration
    
    
    if (abs (alertTimeHold - System.uptime()) > ALTR_INTVAL ) //To avoid continous alerts
    {
        alertFlag = false;
    ...
    if((levelPer < 25 && levelPer > 10 && !alertFlag))
    {
        alertTimeHold = System.uptime();
        alertFlag = true;
        Particle.publish ("sms_turnon", jsonMsg ,  PRIVATE);
        Particle.publish ("eml_turnon", jsonMsg ,  PRIVATE);
    ...
    if(levelPer > 25 && levelPer <50 && alertFlag)  //aler clear
    { 
        alertFlag = false;
        Particle.publish("Alert Clear", "Water level regained ( > 25% )" , PRIVATE);
        return 3; //alert code 3: alert clear : tank level back to  25%
    }
    
    if(levelPer > 50 && !alertFlag )
    {
        alertTimeHold = System.uptime();
        alertFlag = true;
        
        Particle.publish ("sms_turnoff", jsonMsg , PRIVATE);
        Particle.publish ("eml_turnoff", jsonMsg , PRIVATE);
